...
| Asset | Intended use | In this repo |
| --- | --- | --- |
| `shrpware-wordmark.svg` | Full wordmark on dark backgrounds | yes |
| `shrpware-wordmark-dark.svg` | Full wordmark on light backgrounds | no |
| `shrpware-mark.svg` | Angular S on its graphite field; avatars and compact publisher marks | no |
| `shrpware-mark-transparent.svg` | Acid-green Angular S when the background is already controlled | no |
| `../favicon.svg` | Website/browser use only | yes |

Only the wordmark and the favicon are carried here — the site is the only thing
that needs them. The other variants are described so the intent survives; ask
the owner for the master before recreating one, and never rebuild the Angular S
by hand.

`public/og-v5.png` is the ShrpWare link-sharing card, referenced from
`app/layout.tsx`. It is not an app icon or splash screen.

## WavePlume asset package

`public/apps/waveplume/app-assets/` was a staging area: the icon masters and
platform packages were parked in the website repo so they could be carried into
the apps. That handover is done, and the package was removed in `de7527a` —
only `png/waveplume-128.png` remains, because the product page renders it.

The apps now own their icons (`Resources/AppIcon.icns` on macOS, and the
Windows equivalent in `WavePlume-Windows/`). Treat those as the masters. Do not
re-stage a copy here; a second copy is how the two drift apart.
...
for macOS and Windows. The Windows edition is available in Microsoft Store.
DeltaTxt is presented as a native editor for code, large logs, comparison,
merge, and script troubleshooting; its Windows edition is ahead of the active
macOS preview.

Do not invent purchase buttons, prices, release dates, or download URLs. The
approved values are already recorded in `app/lib/site.ts`, which is the source
of truth for versions and destinations — read them from there rather than
copying them into documentation, where they go stale.
