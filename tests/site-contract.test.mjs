...
test("lists 0.3.3 as the current Mac release, without run or debug claims", async () => {
  const product = await fetchPath("/apps/deltatxt", { headers: { accept: "text/html" } });
  const productHtml = await product.text();
  // Apple approved 0.3.3 on 2026-09-25; the listing sells it.
  assert.match(productHtml, /New in 0\.3\.3: Markdown preview/);
  assert.doesNotMatch(productHtml, /App Store review/);
  ...
  assert.match(mac, /The current Mac App Store edition/);
  assert.doesNotMatch(mac, /in review|offers 0\.3\.1/);
  assert.match(mac, /class="button[^"]*"[^>]*>Mac App Store</);
...
test("describes DeltaTxt for search with per-edition structured data", async () => {
  ...
  // The node describes what the App Store sells today.
  assert.equal(mac.softwareVersion, "0.3.3");
  assert.match(JSON.stringify(mac.featureList), /Markdown preview/);
  assert.equal(mac.screenshot.length, 7);
