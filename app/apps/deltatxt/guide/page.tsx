...
export default function DeltaTxtGuide() {
  return <PageShell accent="deltatxt"><ArticleHero title="DeltaTxt for Mac: the guide." intro="Every area of the app, what each preference does, and how to drive DeltaTxt from Finder, Shortcuts, a link, or a shell." />
    <article className="article-body wrap"><BackToApp href="/apps/deltatxt">Back to DeltaTxt</BackToApp>
      ...
      <h2>Opening files, encodings, and large files</h2>
      ...
      <p>Three size thresholds apply and are easy to confuse. Over <strong>16 MB</strong> a file loads behind a cancellable progress sheet so the window appears immediately. Over your <strong>warning threshold</strong>, 64 MB by default, DeltaTxt asks first and offers to search the file instead of loading it. Over <strong>64 MB</strong> crash recovery stops snapshotting the document, and the status bar says so. For files past comfortable editing, <strong>Search ▸ Find in File…</strong> and <strong>Compare ▸ Streaming Compare…</strong> both read from disk and have no size limit.</p>
      ...
      <h2>Editing</h2>
      <p><strong>Edit ▸ Line</strong> holds the commands that treat lines as units: duplicate, move up and down, join, split, four sorts, reverse, remove empty lines, remove duplicates, and three trims. Sorting a file that ends in a newline leaves that newline at the end.</p>
      ...
      <h2>Running and debugging Python</h2>
      <p>Direct download only. <strong>Run ▸ Run</strong> streams output into the panel below the editor, and the last of a process’s output is delivered before its exit is reported, so the final line of a script that fails immediately is never lost. When a run finishes while DeltaTxt is not the frontmost application, a notification reports the exit code.</p>
      <p>Click the gutter to set a breakpoint and use <strong>Run ▸ Debug</strong>. Continue is F8, Step Over F10, Step Into F11, Step Out Shift-F11, and the panel shows the console, Locals and the call Stack. The debugger is Python-first and speaks the standard <code>pdb</code> protocol; conditional breakpoints and watch expressions are not provided.</p>
      ...
      <p><strong>View ▸ Follow File</strong> tails a growing log in place. A multi-byte character split across two writes is held until it is complete, and a rotated log is noticed: DeltaTxt keeps what it has, inserts a marker, and continues with the new file rather than stopping or duplicating everything.</p>

      <h2>Configuring DeltaTxt</h2>
      ...
      <h2>Driving DeltaTxt from outside</h2>
...
deltatxt://merge?base=/abs/b&left=/abs/l&right=/abs/r`}</code></pre>
      ...
      <h2>The deltatxt command, and git</h2>
