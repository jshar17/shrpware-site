...
export default function DeltaTxtChangelog() {
  return <PageShell><ArticleHero eyebrow="DeltaTxt / Changelog" title="DeltaTxt release notes." intro="Current version, download, and release information." />
    <article className="article-body wrap"><BackToApp href="/apps/deltatxt">Back to DeltaTxt</BackToApp><div className="release-entry"><div><span>Current</span><time>2026</time></div><section><h2>DeltaTxt 0.3.1</h2><p>The current Windows x64 release is available from the Microsoft Store or as a digitally signed and timestamped standalone installer.</p><ul><li>Focused Python editing, interpreter discovery, Run, and breakpoint debugging with stepping, Locals, and Stack.</li><li>Find and replace in a document, across a workspace, or through bounded-memory large-file tools.</li><li>Side-by-side comparison, inline changes, review filters, hunk application, unified diff export, and three-way merge.</li><li>Local preferences and recovery with no account, telemetry, advertising, or automatic crash upload.</li></ul><div className="inline-actions"><Link className="button button-primary" href={links.deltatxt.download}>Download installer</Link>{releaseStatus.deltaStoreReady ? <Link className="button button-secondary" href={links.deltatxt.store}>Microsoft Store</Link> : null}</div><p className="fine-print">Standalone installer size: approximately 70 MB.</p><p className="checksum"><strong>SHA-256</strong><code>984477D66D40EB1ABEFEBC418079DF88ECFD504ECB55553185DE83B8AA5DE023</code></p></section></div></article>
  </PageShell>;
}
