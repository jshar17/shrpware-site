...
for index, (filename, slug, title, subtitle) in enumerate(shots, 1):
    ...
    canvas.save(GALLERY / f"windows-{slug}-20260906.webp", quality=91, method=6)
    canvas.resize((640,360), Image.Resampling.LANCZOS).save(GALLERY / f"windows-{slug}-20260906-thumb.webp", quality=85, method=6)
    print(f"{slug}: {shot.size} -> 1920x1080")
Image.open(SOURCE / "02-transcript.png").convert("RGB").save(ROOT / "public/apps/waveplume/hero-windows-transcript-20260906.webp", quality=94, method=6)
